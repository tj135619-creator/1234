...
const HandleSilence =  ({ onNext, lesson }) => {
  ...
  const [comfortZoneSize, setComfortZoneSize] = useState(40);
      ...
      <div className="p-4 md:p-6 lg:p-10 max-w-6xl mx-auto">
        ...
        <div className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 backdrop-blur-md rounded-3xl border-2 border-purple-500/30 shadow-2xl p-6 md:p-8 mb-6 md:mb-8">
          ...
          <div className="flex items-center justify-center mb-6 relative h-64 md:h-80">
            ...
            {/* Inner circle - Comfort Zone (animated) */}
            <div 
              className={`rounded-full bg-gradient-to-br from-purple-600/40 to-pink-600/40 backdrop-blur-sm border-4 border-purple-400/50 shadow-2xl flex items-center justify-center transition-all duration-1000 ${breathingPhase === 'inhale' ? 'scale-105' : 'scale-100'}`}
              style={{ 
          ...
          <button
            ...
            className="w-full px-6 py-6 md:py-8 bg-gradient-to-r from-purple-600 to-pink-600 rounded-3xl font-bold text-lg md:text-xl hover:from-purple-500 hover:to-pink-500 transition-all shadow-xl hover:shadow-2xl hover:scale-105 flex items-center justify-center gap-3"
          >
            <Flame className="w-6 h-6 md:w-8 md:h-8" />
            Log an Attempt (Failed or Not!)
            <Flame className="w-6 h-6 md:w-8 md:h-8" />
          </button>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
          <div className="bg-gradient-to-br from-purple-800/60 to-purple-900/60 backdrop-blur-sm p-5 md:p-6 rounded-3xl border-2 border-purple-500/30 shadow-xl">
            <div className="flex items-center gap-2 mb-3">
              <Trophy className="w-5 h-5 md:w-6 md:h-6 text-yellow-400" />
              <span className="text-purple-300 text-xs md:text-sm font-medium">Attempts</span>
            </div>
            <p className="text-4xl md:text-5xl font-bold text-white mb-1">{failureCount}</p>
            <p className="text-purple-400 text-xs md:text-sm">Times you tried</p>
          </div>

          <div className="bg-gradient-to-br from-pink-800/60 to-purple-900/60 backdrop-blur-sm p-5 md:p-6 rounded-3xl border-2 border-pink-500/30 shadow-xl">
            <div className="flex items-center gap-2 mb-3">
              <Lightbulb className="w-5 h-5 md:w-6 md:h-6 text-orange-400" />
              <span className="text-purple-300 text-xs md:text-sm font-medium">Lessons</span>
            </div>
            <p className="text-4xl md:text-5xl font-bold text-white mb-1">{failureCount}</p>
            <p className="text-purple-400 text-xs md:text-sm">Learned</p>
          </div>

          <div className="bg-gradient-to-br from-indigo-800/60 to-purple-900/60 backdrop-blur-sm p-5 md:p-6 rounded-3xl border-2 border-indigo-500/30 shadow-xl">
            <div className="flex items-center gap-2 mb-3">
              <Award className="w-5 h-5 md:w-6 md:h-6 text-purple-400" />
              <span className="text-purple-300 text-xs md:text-sm font-medium">Resilience</span>
            </div>
            <p className="text-2xl md:text-3xl font-bold text-white mb-1">{resilienceLevel.emoji}</p>
            <p className="text-purple-400 text-xs md:text-sm">{resilienceLevel.label}</p>
          </div>

          <div className="bg-gradient-to-br from-purple-800/60 to-indigo-900/60 backdrop-blur-sm p-5 md:p-6 rounded-3xl border-2 border-purple-500/30 shadow-xl">
            <div className="flex items-center gap-2 mb-3">
              <TrendingUp className="w-5 h-5 md:w-6 md:h-6 text-emerald-400" />
              <span className="text-purple-300 text-xs md:text-sm font-medium">Growth</span>
            </div>
            <p className="text-4xl md:text-5xl font-bold text-white mb-1">{comfortZoneSize}%</p>
            <p className="text-purple-400 text-xs md:text-sm">Zone expanded</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-900/50 to-indigo-900/50 backdrop-blur-md rounded-3xl border-2 border-purple-500/30 shadow-2xl p-6 md:p-8 mb-6 md:mb-8">
         ...
         {/* NEXT BUTTON - SHOULD CALL onNext */}
      <div className="flex gap-4">
        <button
          onClick={() => {
            if (onNext) {
              onNext(); // This calls SkillNavigator.nextSkill()
            }
          }}
